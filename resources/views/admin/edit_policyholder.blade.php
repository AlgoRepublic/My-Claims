                ...
                <form method="post" action="{{ url('/policyHolder/edit') }}">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control" pattern="[A-Za-z0-9\s]+" title="Only alphanumeric characters are allowed | No spaces are allowed." value="{{ old('name', $userData->name) }}" placeholder="John Deo" name="name" required>
                        </div>
                        <input type="hidden" value="{{ old('id', $userData->id) }}" name="id" />
                        <div class="form-group col-md-6">
                            <label for=" ">Surname<span class="error-text">*</span></label>
                            <input type="text" class="form-control" pattern="[A-Za-z0-9\s]+" title="Only alphanumeric characters are allowed | No spaces are allowed." value="{{ old('surname', $userData->surname) }}" placeholder="Deo Smith" name="surname" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for=" ">Cell Number<span class="error-text">*</span></label>
                            <input type="text" data-original="{{ $userData->mobile }}" data-type="mobile" pattern="\d*" maxlength="10" minlength="10" title="10 digits minimum" class="form-control" id="reg-contact-no" value="{{ old('mobile', $userData->mobile) }}" placeholder="0123456789" name="mobile" required>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" data-original="{{ $userData->identity_document_number }}" id="user-idn" data-type="identity_document_number" pattern="\d*" maxlength="13" minlength="13" title="Only dgits are allowed | Length should be 13." class="form-control" value="{{ old('identity_document_number', $userData->identity_document_number) }}" placeholder="123456789" name="identity_document_number" required>
